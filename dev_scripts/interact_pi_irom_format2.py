...
class agent():
    ...
    def get_traj_info(self, id, start_idx=0, steps=8,skip=1, experiment=None):
        ...
        num_frames = steps
        # get annotation path
        for file in os.listdir(f"{val_dataset_dir}/{id}"):
            if ".h5" in file:
                annotation_path = f"{val_dataset_dir}/{id}/{file}"
        ...
        if experiment is not None:
            ...
            video_dir = f"{args.dataset_root_path}/{experiment}/{id}"
        else:
            video_dir = f"{args.dataset_root_path}/{args.task_type}/videos/{id}/resized_cv"
...
if __name__ == "__main__":
    from irom_config_format2 import wm_args
