...
class agent():
    ...
    def get_traj_info(self, id, start_idx=0, steps=8,skip=1, experiment=None):
        num_frames = steps
        annotation_path = f"{self.args.val_dataset_dir}/annotation/{id}/{id}.h5"
        data = {}
        ...
        if experiment is not None:
            video_dir = f"{experiment}/videos/{id}/resized_wm"
        else:
            video_dir = f"{self.args.project_root}/{self.args.task_type}/videos/{id}/resized_cv"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_root_path', type=str, default=f"{PROJECT_ROOT}/data/init_conditions")
    ...
    for val_id_i, start_idx_i in zip(args.val_id, args.start_idx):
        ...
        id = val_id_i
        experiment=f"{args.dataset_root_path}/{args.dataset_subdir}"
        eef_gt, joint_pos_gt, video_dict, video_latents, text_i = Agent.get_traj_info(val_id_i, start_idx=start_idx_i, steps=int(pred_step*interact_num+8), experiment=experiment)
