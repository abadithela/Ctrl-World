...
from models.pipeline_ctrl_world_x0_interrupt_diffusion import CtrlWorldDiffusionPipeline
...
class wm_args:
    ...
    dataset_root_path = f"{PROJECT_ROOT}/data/init_conditions"
    dataset_subdir = "pi05_3-distractor" # Figure out why these arguments are not clear!
    save_root_path = f"{PROJECT_ROOT}/interrupt_diffusion/outputs"
    ...
    def __post_init__(self):
        self.save_dir = f"{self.save_root_path}/{self.policy_type}_{self.dataset_subdir}"
        ...
        os.makedirs(self.val_dataset_dir, exist_ok=True)
        os.makedirs(f"{self.val_dataset_dir}/videos", exist_ok=True)
        subfolders = sorted([
...
class agent():
        ...
        try:
            with open(txt_path, 'r') as f:
                for line in f:
                    line = line.strip()
                    if line.startswith("pick object:"):
                        pick_obj = line.split(":", 1)[1].strip().lower()
                    elif line.startswith("place object") and not place_obj:
                        place_obj = line.split(":", 1)[1].strip().lower()
        except FileNotFoundError:
            pass
        # handle edge cases
        if pick_obj == "small red cup":
            pick_obj = "cup"
        if pick_obj == "green bell pepper/capsicum":
            pick_obj = "green bell pepper"
        if pick_obj == "strawberries":
            pick_obj = "strawberry"
        if pick_obj == "box (cake mix)":
            pick_obj = "box of cake mix"
        if pick_obj and place_obj:
            if place_obj in on_list:
                return f"put the {pick_obj} on the {place_obj}"
            else:
                return f"put the {pick_obj} in the {place_obj}"
        elif pick_obj:
            return f"pick {pick_obj}"
        return "pick and place"
    ...
    def load_init_condition(self, env_dir, instruction_override=None, initial_joint_pos=None, initial_eef_pos=None):
        ...
        image_files = ["initial_frame_left.png", "initial_frame_right.png", "initial_frame_wrist.png"]
        for img_name in image_files:
            img_path = env_dir / img_name
            img = np.array(Image.open(img_path).convert("RGB"))  # (H, W, 3)
            img = cv2.resize(img, (320, 192))  # (192, 320, 3)
        ...
        if initial_joint_pos is not None and initial_eef_pos is not None:
            ...
            if not h5_files:
                h5_file = f"{self.args.PROJECT_ROOT}/data/pi05_demos/annotation/00/00.h5"
                print("Annotation path not saved; using default at: ", h5_file)
            else:
                h5_file = h5_files[0]
            ...
            eef_pos = np.array(data['cartesian_pose'])
            joint_pos = np.array(data['joint_position'])
            gripper_pose = np.array(data["gripper_pose"])

            if eef_pos.ndim == 2 and eef_pos.shape[0] >= 1:
                eef_pos = eef_pos[0]
            if joint_pos.ndim == 2 and joint_pos.shape[0] >= 1:
                joint_pos = joint_pos[0]
            if gripper_pose.ndim == 2 and gripper_pose.shape[0] >= 1:
                gripper_pose = gripper_pose[0]
            
            try:
                eef_pos = np.concatenate((eef_pos,gripper_pose),axis=1)
                joint_pos = np.concatenate((joint_pos, gripper_pose), axis=1)
            except:
                eef_pos = np.concatenate((eef_pos,gripper_pose))
                joint_pos = np.concatenate((joint_pos, gripper_pose))
    ...
    def forward_wm(self, action_cond, video_latent_cond, his_cond=None, text=None):
        ...
        with torch.no_grad():
            ...
            st = time.time()
            _, intrm_frames, latents, intrm_latents = CtrlWorldDiffusionPipeline.__call__(
                pipeline,
                image=image_cond,
                text=text_token,
                width=args.width,
        ...
        for i in range(len(intrm_latents)):
            intrm_latents[i] = einops.rearrange(intrm_latents[i], 'b f c (m h) (n w) -> (b m n) f c h w', m=3,n=1) # (B, 8, 4, 32,32)

        # decode predicted video
        decoded_video = []
        intrm_decoded_videos = [[] for i in range(len(intrm_latents))]
        bsz, frame_num = latents.shape[:2]
        intrm_bszs, intrm_frame_nums = [None] * len(intrm_latents), [None] * len(intrm_latents)
        for i in range(len(intrm_latents)):
            intrm_bszs[i], intrm_frame_nums[i] = intrm_latents[i].shape[:2]

        x = latents.flatten(0, 1)
        intrm_xs = [None] * len(intrm_latents)
        for i in range(len(intrm_latents)):
            intrm_xs[i] = intrm_latents[i].flatten(0, 1)

        decode_kwargs = {}
        intrm_decode_kwargs = [{} for i in range(len(intrm_latents))]
        ...
        for i in range(0, x.shape[0], args.decode_chunk_size):
            ...
            for j in range(len(intrm_latents)):
                intrm_chunk = intrm_xs[j][i:i+args.decode_chunk_size]/pipeline.vae.config.scaling_factor
                intrm_decode_kwargs[j]["num_frames"] = intrm_chunk.shape[0]
                vae_dtype = next(pipeline.vae.parameters()).dtype
                intrm_decoded_videos[j].append(pipeline.vae.decode(intrm_chunk.to(vae_dtype), **intrm_decode_kwargs[j]).sample)
        ...
        intrm_videos = []
        for i in range(len(intrm_decoded_videos)):
            intrm_videos.append(torch.cat(intrm_decoded_videos[i], dim=0))
            intrm_videos[i] = intrm_videos[i].reshape(intrm_bszs[i], intrm_frame_nums[i],*intrm_videos[i].shape[1:])
            intrm_videos[i] = ((intrm_videos[i] / 2.0 + 0.5).clamp(0, 1)*255)
            intrm_videos[i] = intrm_videos[i].detach().to(torch.float32).cpu().numpy().transpose(0,1,3,4,2).astype(np.uint8)

        videos_cat = np.concatenate([v for v in videos], axis=2)  # width axis
        intrm_videos_cat = np.concatenate(intrm_videos, axis=-3)
        intrm_videos_cat = np.concatenate([intrm_video for intrm_video in intrm_videos_cat], axis=-2).astype(np.uint8)
        return videos_cat, intrm_videos_cat, videos, latents, intrm_videos, intrm_latents
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clip_model_path', type=str, default=f"{PROJECT_ROOT}/clip")
    parser.add_argument('--ckpt_path', type=str, default=f"{PROJECT_ROOT}/checkpoints/checkpoint-10000.pt")
    parser.add_argument('--svd_model_path', type=str, default=f"{PROJECT_ROOT}/svd")
    parser.add_argument('--save_root_path', type=str, default=None)
    parser.add_argument('--task_type', type=str, default=None)
    parser.add_argument('--policy_type', type=str, default=None)
    parser.add_argument('--pi_ckpt', type=str, default=f"{PROJECT_ROOT}/openpi/checkpoints/openpi-assets/checkpoints/pi05_droid")

    # init condition args
    parser.add_argument('--init_conditions_dir', type=str,
                        default=f"{PROJECT_ROOT}/data/init_conditions/envs",
    ...
    parser.add_argument('--num_chunks', type=int, default=70,
                        help="Total number of chunks to split env_dirs into.")
    parser.add_argument('--chunk_idx', type=int, default=0,
                        help="0-based index of the chunk this job should process.")
    parser.add_argument('--num_trials', type=int, default=3,
                        help="Number of times to run each environment")
    ...
    args.save_dir = f"{args.save_root_path}/{args.policy_type}_{args.dataset_subdir}"
    ...
    # split into chunks for parallel SLURM array jobs
    if args_new.num_chunks > 1:
        chunks = [env_dirs[i::args_new.num_chunks] for i in range(args_new.num_chunks)]
        env_dirs = chunks[args_new.chunk_idx]
    print(f"Chunk {args_new.chunk_idx}/{args_new.num_chunks}: running {len(env_dirs)} environments: {[p.name for p in env_dirs]}")
    ...
    num_trials = args_new.num_trials
    print("beginning rollouts...")

    for env_dir in env_dirs:
        env_name = env_dir.name
        for trial_idx in range(num_trials):
            if os.path.isdir(f"{args.save_dir}/{env_name}/video"):
                files = os.listdir(f"{args.save_dir}/{env_name}/video")
                num_files = len(files)
                if len(files) >= num_trials * 2:
                    print(f"Skipping {env_name} trial {trial_idx+1}/{num_trials} because {num_files} files already exist in {args.save_dir}/{env_name}/video")
                    continue
                else:
                    # run only upto num_trials - num_files trials
                    if trial_idx == num_files / 2:
                        print(f"Running {env_name} trial {trial_idx+1}/{num_trials} (only {num_trials - num_files} trials will be run due to existing files in {args.save_dir}/{env_name}/video)")
                    else:
                        print(f"Skipping {env_name} trial {trial_idx+1}/{num_trials} because {num_files} files already exist in {args.save_dir}/{env_name}/video")
                        continue
            print(f"\n{'='*60}")
            print(f"Environment: {env_name}, Trial: {trial_idx+1}/{num_trials}")
            ...
            video_to_save, intrm_video_to_save, info_to_save = [], [], []
            ...
            for i in range(interact_num):
                ...
                videos_cat, intrm_videos_cat, video_dict_pred, predict_latents, intrm_video_dict_pred, intrm_predict_latents = Agent.forward_wm(
                    action_cond,
                    current_latent,
                    his_cond=his_latent,
                ...
                video_to_save.append(videos_cat[:pred_step-1])  # save exterior view
                intrm_video_to_save.append(intrm_videos_cat[:pred_step-1])
                info_to_save.append(policy_in_out)

            # save rollout video and info
            print("\n" + "=" * 60)
            video = np.concatenate(video_to_save, axis=0)
            intrm_video = np.concatenate(intrm_video_to_save, axis=0)
            
            text_id = text_i.replace(' ', '_').replace(',', '').replace('.', '').replace("'", '').replace('"', '')[:40]
            uuid = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            filename_video = f"{args.save_dir}/{env_name}/video/time_{uuid}_trial{trial_idx}_{text_id}.mp4"
            intrm_filename_video = f"{args.save_dir}/{env_name}/video/intrm_time_{uuid}_trial{trial_idx}_{text_id}.mp4"
            os.makedirs(os.path.dirname(filename_video), exist_ok=True)
            os.makedirs(os.path.dirname(intrm_filename_video), exist_ok=True)
            mediapy.write_video(filename_video, video, fps=4)
            mediapy.write_video(intrm_filename_video, intrm_video, fps=4)
            print(f"Saving video to {filename_video}")
            print(f"Saving intrm video to {intrm_filename_video}")

            info = {'success': 1, 'env': env_name, 'trial': trial_idx, 'instructions': text_i}
            for key in info_to_save[0].keys():
                info[key] = []
                for j in range(len(info_to_save)):
                    info[key] += info_to_save[j][key].tolist()
            filename_info = f"{args.save_dir}/{env_name}/info/time_{uuid}_trial{trial_idx}_{pred_step}_{text_id}.json"
