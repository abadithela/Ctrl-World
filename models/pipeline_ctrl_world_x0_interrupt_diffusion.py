    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if (i+1) % 5 == 0:
                # if (32 <= i <= 39):
                    intrm_latents.append(latents_x0.clone())
                    # DEBUG: print(f"appended a latent to intrm_latents. i = {i}")
        ...
        if not output_type == "latent":
            ...
            for i in range(len(intrm_latents)):
                intrm_latents[i] = intrm_latents[i].to(self.vae.dtype)
                intrm_frames.append(self.decode_latents(intrm_latents[i], num_frames, decode_chunk_size))
                intrm_frames[i] = svd_tensor2vid(intrm_frames[i], self.video_processor, output_type=output_type)
        else:
            frames = latents
            intrm_frames = intrm_latents

        self.maybe_free_model_hooks()

        if not return_dict:
            return frames,intrm_frames,latents,intrm_latents
